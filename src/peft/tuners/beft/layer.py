...
class BEFTLayer(BaseTunerLayer):
    ...
    def update_layer(self, adapter_name, init_beft_weights, inference_mode: bool = False, **kwargs):
        weight = torch.randn((1, self.out_features))
        self.beft_bias[adapter_name] = nn.Parameter(weight)
        if init_beft_weights:
...
class Linear(nn.Module, BEFTLayer):
    ...
    def __init__(
        ...
        init_beft_weights: bool = True,  # whether to initialize BEFT weights
        **kwargs,
    ) -> None:
        super().__init__()
        ...
        self.update_layer(adapter_name, init_beft_weights)
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.beft_bias.keys():
                ...
                if safe_merge:
                    ...
                    base_layer.bias.data = (base_layer.bias.data + BEFT_bias).clone().to(orig_dtype)
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                if active_adapter not in self.beft_bias.keys():
                    continue
                dtype = self.beft_bias[active_adapter].dtype
                BEFT_bias = BEFT_bias + self.beft_bias[active_adapter].flatten()

            result = self.base_layer(x, *args, **kwargs)
            result_dtype = result.dtype
            result = (result + BEFT_bias).to(result_dtype)
